...
классификатора. Модель заявки (CM-1, миграция `c6d7e8f9a0b1`), жизненный цикл —
переходы статусов, права, guard удаления (CM-2) — и прокси черновика в
классификатор (CM-5) реализованы ([[Тикеты: предтариф как черновик тарифа]]).
...
payload. `approved` означает решение финдиректора, но не публикацию. Отдельный nullable
`published_at` фиксирует подтверждённую публикацию перед созданием заказа; миграция
`b1c2d3e4f5a6` заполняет его историческим `approved`-заявкам с тарифным pk. CHECK разрешает
marker только у `approved` со ссылкой на классификатор.
...
В JSONB `offer.tariffs` связанная позиция хранит строковый `pre_tariff_id`; внешний
`is_draft_tariff` не сохраняется в snapshot и перечитывается из классификатора. Привязка
оффера и экономическая правка/удаление заявки сериализованы PostgreSQL advisory-lock по UUID.
Подробнее — [[Черновик тарифа публикуется при отправке заказа]].
