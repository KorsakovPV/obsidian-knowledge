...
- скидочный маршрут определяется максимальной скидкой среди всех тарифов всех offers;
  скидка считается от прайса классификатора по продавцу (`price` для datafort,
  `beeline_price` для vimpelcom) против цены оффера, приведённой
  `calculate_price_by_seller`; расчёт выполняет сервер при создании/изменении
  оффера — см. [[Sequential KP Approval Tickets]], Ticket 16;
