...
- скидочный маршрут определяется максимальной скидкой среди всех тарифов всех offers;
  скидка считается от прайса классификатора (`tariff_element.price`) против `price`
  тарифа оффера; `partner_client_price` и `beeline_price` в расчёте не участвуют;
  расчёт выполняет сервер при создании/изменении оффера — см.
  [[Sequential KP Approval Tickets]], Ticket 16;
- стадия `product_owner` уходит владельцам продуктов, вызвавших её включение: продукты
  фиксируются в стадии при построении маршрута, а права владельцев ограничиваются
  продуктом через scope назначения — см. Ticket 17;
...
2. Просроченный token не закрывает pending stage; начальный SLA — 3 календарных дня, token TTL — 14 дней. Эскалация просроченной стадии остаётся отдельным продуктовым решением.
