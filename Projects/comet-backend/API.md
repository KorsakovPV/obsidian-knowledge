...
одному на оффер, с идемпотентным ключом; если часть заказов не создана — 409, сделка не
считается переданной. Пустой маршрут — тоже пройденный: такие заказы уходят в ОП
