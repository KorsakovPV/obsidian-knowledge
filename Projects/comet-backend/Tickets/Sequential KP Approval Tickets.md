...
- База скидки — прайс классификатора по продавцу: `tariff_element.price` для
  `datafort`, `tariff_element.beeline_price` для `vimpelcom`. Индивидуальная цена
  клиента в первом приближении не участвует; это возможное улучшение позже.
- Цена оффера для сравнения — `calculate_price_by_seller(partner_client_price, seller)`,
  то есть сырой `partner_client_price` в расчёт не идёт. Для `datafort` значения
  совпадают, для `vimpelcom` цена приводится делением на 1.1 — сравнение однородно.
- Формула: `discount_percent = (base − offer) / base × 100`, база `<= 0` даёт скидку `0`.
...
- Добавить внутреннюю схему хранения тарифа (`OfferTariffStoredSchema`) с полями
  `base_price`, `base_price_source`, `discount_percent`. Публичные
  `OfferCreateSchema` / `OfferUpdateSchema` не менять: фронт по-прежнему шлёт четыре поля.
- В `OfferService.create()` и `update()` до записи обогащать тарифы расчётом скидки по
  прайсу классификатора и продавцу сделки. Route builder не менять: он уже читает
  `discount_percent` и `base_price`.
...
- Тесты: unit расчёта (`datafort` / `vimpelcom`, отсутствие цены в классификаторе,
  база `0`, границы 10/15/20%), интеграционный маршрут по скидке через создание оффера
  со стабом классификатора, пересборка draft при изменении цены.
