...
- Ticket 16 чинит расчёт скидки, без которого скидочный маршрут не строится для
  офферов, созданных через API;
- Ticket 17 адресует стадию владельцам конкретных продуктов.
...
Цель: сделать скидочный маршрут работоспособным для офферов, созданных через API.

Зависимости: Ticket 2 (route builder), Ticket 6 (deal-level snapshot). Это
forward-правка контракта тарифа, а не переоткрытие завершённых тикетов.

Проблема:

- `ApprovalRouteBuilder` вычисляет скидку из самого тарифа: `discount_percent` либо
  `partner_client_price` против `base_price` / `classifier_price` / `beeline_price`.
- Входной контракт `TariffsSchema` хранит только `pk`, `quantity`,
  `partner_client_price`, `group_code`; остальные ключи отбрасываются валидацией
  (`extra="ignore"`), и в `offer.tariffs` они не попадают.
- Следствие: у оффера, созданного через API, максимальная скидка всегда `0`, и стадии
  `presale`, `lead`, `sales_director`, `finance_director` не строятся никогда.
  Через API сейчас воспроизводится только маршрут по особым условиям.

Принятые решения:

- База скидки — прайс классификатора `tariff_element.price`. `beeline_price` — цена для
  посредника, который продаёт продукт со своей наценкой; в расчёте пока не участвует и
  может использоваться позже для отдельных, ещё не описанных случаев.
- Цена оффера для сравнения — `price` тарифа, то есть цена продажи.
  `partner_client_price` — цена конечного потребителя, нужна ему для статистики и в
  бизнес-логике согласования не участвует.
- Поле `price` было убрано из контракта тарифа в DFDEV-1691 и возвращается: его
  присылает фронт. Скидка не зависит от продавца, поэтому конверсия
  `calculate_price_by_seller` в расчёте не применяется — она остаётся контрактом ОП.
- Формула: `discount_percent = (base − price) / base × 100`, база `<= 0` даёт скидку `0`;
  цена выше прайса скидкой не считается.
- Индивидуальная цена клиента в первом приближении не участвует; возможное улучшение позже.
- Тариф, для которого классификатор не вернул цену, считается ошибкой данных: создание
  или изменение оффера отклоняется понятной доменной ошибкой, а не молча даёт скидку `0`.
- Скидку считает сервер и сохраняет в тарифе. Клиент не может прислать
  `discount_percent` или базовую цену: иначе фронт решает, нужно ли согласование.

Что сделать:

- Вернуть `price` в контракт тарифа (`TariffsSchema`) как цену продажи; отсутствие
  цены — доменная ошибка, а не тихая скидка `0`.
- Добавить внутреннюю схему хранения тарифа (`OfferTariffStoredSchema`) с полями
  `base_price`, `base_price_source`, `discount_percent`: их заполняет только сервер.
- В `OfferService.create()` и `update()` до записи обогащать тарифы расчётом скидки по
  прайсу классификатора. Route builder не менять: он уже читает `discount_percent`
  и `base_price`.
- Не выполнять сетевые вызовы внутри workflow-транзакции: расчёт выполняется при
  создании/изменении оффера, а rebuild и start читают уже сохранённые значения.
- Добавить доменную ошибку отсутствующей цены классификатора со стабильным code и
  сообщением, замапить её на 4xx в exception handlers.
- Отдавать `discount_percent` и `base_price` в read-схеме оффера, чтобы UI показывал ту
  же скидку, по которой строится маршрут.
- Добавить идемпотентный бэкофилл (`scripts/backfill_offer_tariff_discounts.py`) для
  офферов сделок без активного согласования. Замороженные `subject_snapshot` не трогать.
- Тесты: unit расчёта (скидка от прайса, цена выше прайса, нулевая база, отсутствие
  `price` у тарифа, отсутствие тарифа в классификаторе), тесты `OfferService` и
  интеграционный маршрут по скидке через создание оффера со стабом классификатора.

DoD:

- Оффер, созданный через API с ценой ниже прайса, строит скидочный маршрут.
- Скидка и база сохранены в тарифе и попадают в `subject_snapshot`, поэтому решение
  согласующего объяснимо постфактум.
- Клиент не может повлиять на скидку через payload.
- Тариф без цены в классификаторе даёт понятную ошибку, а не тихий маршрут без стадий.
- Бэкофилл идемпотентен и не меняет уже согласованные версии.

Эксплуатация: после бэкофилла у части сделок появятся скидочные маршруты, которых
раньше не было, — объём согласований заметно вырастет. Прогонять в одном окне с
cutover, см. [[Approval Cutover Runbook]].

Статус реализации на 2026-07-28 (ветка `DFDEV-2164_16`):

- `OfferTariffStoredSchema` расширяет тариф полями `base_price`, `base_price_source`,
  `discount_percent`; публичные `OfferCreateSchema` / `OfferUpdateSchema` не изменились,
  поэтому клиент по-прежнему присылает четыре поля и на скидку не влияет;
- общие поля вынесены в `OfferFieldsMixin` / `OfferUpdateFieldsMixin`, а запись в БД идёт
  через `OfferCreateDBSchema` / `OfferUpdateDBSchema` — иначе pydantic отбросил бы
  серверные поля при сериализации по объявленному типу;
- `TariffsSchema` снова содержит `price` (цена продажи, nullable в схеме ради понятной
  доменной ошибки вместо 422);
- `TariffDiscountService` считает скидку от `tariff_element.price` классификатора против
  `price` тарифа; цена выше прайса и нулевая база дают `0`; продавец на расчёт не влияет;
- `OfferService.create()` и `update()` обогащают тарифы до записи; перенос оффера в другую
  сделку скидку не меняет. Route builder не изменялся;
- тариф без `price` даёт `OfferTariffPriceRequiredError` → HTTP 400 с code
  `offer_tariff_price_required`, тариф без цены в классификаторе —
  `OfferTariffPriceNotFoundError` → code `offer_tariff_price_not_found`;
- `TariffSchemaOfferOut` отдаёт `base_price` и `discount_percent`, поэтому UI видит ту же
  скидку, по которой построен маршрут;
- бэкофилл `scripts/backfill_offer_tariff_discounts.py` (dry-run по умолчанию) обходит
  офферы сделок без `pending`/`blocked` версии; офферы без `price` в тарифах не
  обновляются и печатаются отдельным списком;
- порядок выката: фронт должен начать присылать `price` до включения бэкенда, иначе
  создание оффера будет отклоняться с `offer_tariff_price_required`;
- тесты: `tests/services/test_tariff_discount.py`, `tests/services/test_offer.py` и
  интеграционные сценарии в `tests/integration/test_approval_routes_db.py` (маршрут по
  скидке через `OfferService` и отсутствие маршрута при цене на уровне прайса);
- `scripts/approval_route_probe.py` больше не сообщает о невозможности скидочного
  маршрута: через API он теперь воспроизводится.

## Ticket 17 — Выбирать согласующих стадии по продуктам, вызвавшим её включение

Цель: стадия `product_owner` должна уходить владельцам конкретных продуктов, а не всем
holder-ам permission.

Зависимости: Ticket 5 (assignee selector), Ticket 6 (deal-level маршрут и snapshot).

Проблема: `ApprovalStageAssigneeSelector` знает о стадии только `required_permission`,
поэтому групповая стадия рассылается всем пользователям с этим правом. Для владельца
продукта это неверно: сделка может содержать несколько продуктов с разными владельцами,
и письмо получают люди, к предмету согласования отношения не имеющие.

Принятые решения:

- Механика — групповая стадия (вариант A): письмо получают владельцы затронутых
  продуктов, первый валидный ответ закрывает стадию. Кворум (обязательный ответ каждого
  владельца) не вводится: он потребовал бы участников стадии, токена на человека и
  переписанной state machine.
- Состав получателей определяется **причиной включения стадии**: особые условия —
  продукты офферов с непустыми `special_conditions`; скидка — продукты, чья скидка
  включила стадию. Сегодня `product_owner` включается только особыми условиями.
- Зона ответственности хранится как scope назначения права:
  `lkm_user_permissions.scope_product_id` nullable. `NULL` означает право на все
  продукты, поэтому существующие назначения продолжают работать без изменений.
- Набор продуктов стадии фиксируется при построении маршрута и замораживается вместе с
  ним: активация может произойти позже, и повторный вывод причины дал бы другой состав.
- Единоличные стадии (директора, юрист) scope не используют: они не привязаны к продукту.

Что сделать:

- Добавить `lkm_user_permissions.scope_product_id` с уникальностью
  `(user_id, permission_id, scope_product_id)` и поддержать scope в админском API прав.
- Добавить `approval_stages.scope_product_ids` (JSONB): route builder заполняет его
  продуктами, вызвавшими включение стадии.
- Расширить `lkm_user_crud.list_effective_permission_holders(codename, product_ids)`:
  holders без scope плюс holders со scope из списка. Пустой список сохраняет текущее
  поведение.
- `ApprovalStageAssigneeSelector` читает `stage.scope_product_ids`; контракт
  `select_for_stage(stage)` не меняется.
- В ошибку выбора согласующего и в `approval.blocked_permission` добавить продукты, для
  которых владелец не найден: сейчас видно только permission.
- Тесты: владелец со scope, глобальный holder без scope, два продукта с разными
  владельцами (оба получают письмо, первый ответ закрывает стадию), отсутствие владельца
  даёт `blocked`, скидочные и единоличные стадии scope игнорируют.

DoD:

- Стадия `product_owner` уходит только владельцам продуктов, вызвавших её включение,
  и глобальным holder-ам.
- Состав получателей воспроизводим по сохранённому маршруту и не зависит от момента
  активации.
- Отсутствие владельца продукта даёт восстановимый `blocked` с понятной причиной.
- Остальные стадии и state machine не меняются.

Отложено (влияет на будущие правки, поэтому расчёт держать в одном месте):

- Скидку возможно переведут с процентов на конкретную сумму — метрика считается в
  `TariffDiscountService` и `ApprovalRouteBuilder._resolve_discount_stage_codes`,
  замена должна оставаться локальной.
- Стадию директора возможно будут включать по сумме скидок всех продуктов, а не по
  максимальной. Это правило маршрута, а не адресации: ляжет в builder и на Ticket 17
  не влияет.

Статус реализации на 2026-07-28 (ветка `DFDEV-2164_17`):

- миграция `a7b8c9d0e1f3` добавляет `lkm_user_permissions.scope_product_id` и
  `approval_stages.scope_product_ids`; прежний `UNIQUE(user_id, permission_id)` заменён
  двумя частичными индексами, потому что NULL в Postgres не сравнивается сам с собой и
  обычный ключ не запретил бы два глобальных назначения;
- route builder заполняет `scope_product_ids` только у `product_owner` продуктами офферов
  с непустыми особыми условиями; скидочные и единоличные стадии остаются без scope, их
  адресация не изменилась. В `route_context` добавлен `special_conditions_product_ids`;
- `lkm_user_crud.list_effective_permission_holders(codename, product_ids)` отбирает
  назначения без scope плюс назначения на перечисленные продукты; пустой список сохраняет
  прежнее поведение;
- `ApprovalStageAssigneeSelector` читает `stage.scope_product_ids`, контракт
  `select_for_stage(stage)` не изменился; в `ApprovalAssigneeSelectionError` добавлены
  `product_ids`, поэтому по `blocked` видно, для какого продукта нет владельца;
- админское API прав принимает `scoped_permissions` (`codename` + `product_id`) рядом с
  прежним `permission_codenames`; старый payload продолжает работать;
- тесты: builder (scope только у product_owner, дедупликация продуктов), selector
  (передача scope, стадия без scope, единоличная игнорирует scope), интеграционные на
  PostgreSQL (письмо получают владелец нужного продукта и глобальный holder, владелец
  другого продукта — нет; отсутствие владельца даёт ошибку активации с `product_ids`).
