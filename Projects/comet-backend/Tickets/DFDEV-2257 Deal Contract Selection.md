...
| Этап | Тикеты | Что даёт | Блокер |
|---|---|---|---|
| 1 | 1–3 | Фронт может показать список договоров клиента с типом | решение по `INNER_DF_CONTRACT` (Ticket 1) |
| 2 | 4–7 | Выбор договора на создании, неизменяемость, тип в сделке | этап 1 |
| 3 | 8–9 | Легаси-сделки, тесты, документация контракта | этап 2 |
...
2. **Валидация продавца.** Проверять ли на создании, что
   `contract.parent_client_id == SELLER_PARENT_CLIENT_IDS[deal.seller]`? Строгая
   проверка исключает выбор договора «не того» продавца, но может отсечь валидные
   случаи (договор с другим родителем, партнёрская схема). Рекомендация — включить
   проверку, раз список для выбора и так фильтруется по продавцу.
3. **`is_deleted` в customers.** Отдаёт ли customers это поле в GET-ответе договора?
   От этого зависит выбор способа валидации в Ticket 4.
4. **Номер договора во фронте.** Нужен ли `contract_number` в `DealSchema`, или
   фронт подтягивает договор отдельным запросом? От этого зависит объём Ticket 7.
