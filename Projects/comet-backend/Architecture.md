...
- Ролевая модель ЛКМ (LKM): роли `manager / presale / sales_lead / sales_director / finance_director / product_owner / lawyer / admin`,
  ...
  существующей legacy-записи эта ручка дополняет отсутствующий `email` из токена и
  заменяет пустой либо равный `ad_login` `full_name` на подтверждённое имя из Keycloak,
  не перезаписывая уже заполненный профиль. Для
...
- **Наружу неполный состав не уходит:** и `request_approval()`, и `implement_deal()` перед
  работой зовут `_ensure_offers_renderable()` — одним запросом на сделку проверяют, что все
  позиции офферов ещё есть в классификаторе, и отвечают 422 `offer_tariffs_missing` с
  перечнем. Проверка стоит **до** снапшота: неполный состав не должен попасть в
  `subject_snapshot`. Обе точки нужны — сделка с пустым маршрутом на согласование не ходит,
  версия готовится прямо в имплементации.
...
- **Отзыв согласования** доступен в `pending` и `blocked` (DFDEV-2264). Кроме него,
  состояние `blocked` снова открывает `request_approval`: повторный запрос вызывает
  `retry_activation` для той же waiting-стадии без пересборки маршрута. В `pending`
  повторная отправка по-прежнему закрыта.
...
Продукты классификатора читаются в двух режимах. `ProductService.get_products_names()`
сводит все версии к одной актуальной на `product_line_id`, а чтение по точному
`product_id` возвращает и архивную версию. Оффер хранит конкретный `product_id`, а
`product_line_id` и `product_archived_at` обогащаются на чтении. Если обязательную
метаинформацию получить нельзя, карточка отвечает 502 вместо ложного 200. Полные
правила — [[DFDEV-2491 Архивные продукты]].

| Сервис | Назначение |
|--------|-----------|
| `classifier.py` (`ClassifierService`) | Классификатор продуктов/услуг (в коде и логах — **ServiceCatalog**), вход по сервисной УЗ через `/api/ldap_login/` под общим локом: токен один на клиент, и параллельные запросы не логинятся наперегонки. Чтение: продукты, группы, тарифы по pk, справочники формы черновика. **Запись** (с 25.08.2026): создание, правка, публикация и удаление черновика тарифа — только через `draft_tariff.py`, который разводит отказы классификатора по кодам и сбрасывает кеш дерева справочников. Особенность: **каждый рендер оффера ходит в классификатор по pk** — названия, единицы, тип потребления и валюта приходят из `enrich_tariff_data`. Ненайденный pk с 26.08.2026 карточку не роняет: позиция пропускается с пометкой `tariff_warnings`, суммы помечаются неполными, а критичные операции (отправка на согласование и имплементация) блокируются перечнем позиций. Полная недоступность классификатора — 502, не 500. Чтение тарифов по pk идёт с явным `page_size` и дробится по 100 pk: выдача классификатора пагинируется по 10, и раньше состав больше десяти позиций молча обрезался |
| `tariff_classifier.py` (`TariffClassifierService`) | Агрегат справочников для формы черновика тарифа: дерево ветки услуг склеивается из четырёх плоских списков, словари читаются параллельно. Кеш в Redis на час, пересборка под процессным локом. После неудачной пересборки процесс на 30 секунд запоминает `ClassifierUnavailableError`: запросы без готового кеша получают тот же 502 без новых девяти обращений к классификатору; успешный кеш или пересборка сбрасывают cooldown. Состояние не разделяется между воркерами — [[DFDEV-2452 Защита классификатора от повторных запросов после сбоя]], [[API]] |
| `draft_tariff.py` (`DraftTariffService`) | Единственный путь Кометы к записи в классификатор: черновик заводится, правится, публикуется и удаляется отсюда. Отказы разведены по кодам (`rejected`, `publish_conflict`, `draft_missing`), сеть и 5xx — 502 |
