...
| Сервис | Назначение |
|--------|-----------|
| `classifier.py` (`ClassifierService`) | Классификатор продуктов/услуг (в коде и логах — **ServiceCatalog**), вход по сервисной УЗ через `/api/ldap_login/`. Только чтение: продукты, группы, тарифы по pk. Особенность: **каждый рендер оффера ходит в классификатор по pk** — названия, единицы, тип потребления и валюта приходят из `enrich_tariff_data`, поэтому ненайденный pk сегодня роняет всю карточку оффера (`TariffCountNotMatchClassifierError`); план смягчения и запись черновиков тарифов — [[Предтариф как черновик тарифа]] |
