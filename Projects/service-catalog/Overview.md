...
- [[Уровни цен тарифов]] — фундамент новой матрицы согласования: модель
  хранения, идемпотентная загрузка из Excel, changelog, защита `price_code` и
  план проверок. Статус: спроектировано, не реализовано.
- [[Draft-tariff]] — предтариф как черновик тарифа (`is_draft_tariff`): что
  требуется доработать в классификаторе, ловушки реализации, открытые вопросы
  и план работ — тикеты CL-1…CL-4 с деталями до файлов. Ресерч со стороны
  comet-backend, DFDEV-2282.

Папки `docs/` в репозитории нет — переносить из репозитория пока нечего.
