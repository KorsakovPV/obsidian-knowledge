...
| --- | --- | --- |
| `/api/v2/listapi/` | `listapi` | расширяемый read-only V2 для сторонних сервисов |
...
| `/api/products/` | `ProductListViewSet` | read-only дерево продукта с группами и опубликованными тарифами; черновики исключены, кэш в Redis, экшены сброса кэша |
| `/api/products_groups/` | `ProductGroupListViewSet` | список групп продуктов |
| `/api/draft_tariffs/` | `DraftTariffViewSet` | CRUD черновиков; `POST /{pk}/publish/` публикует идемпотентно |

Повтор `POST /api/draft_tariffs/{pk}/publish/` для уже опубликованного активного тарифа
возвращает `200` с актуальной записью. Полный дубль опубликованного тарифа возвращает
`400` с `code: published_duplicate`; код является межсервисным контрактом с Кометой.
Публикация, изменение/удаление черновика и создание обычного тарифа сериализуются общей
транзакционной advisory-блокировкой; публикация дополнительно блокирует строки своей
группы составного кода в порядке `pk`.

Дерево продукта кэшируется под версионированным ключом `Product:v2:{id}`. Предыдущий
ключ `Product {id}` намеренно не читается: его бессрочное содержимое могло включать
черновики до введения фильтра.

При rolling deploy сначала нужно полностью вывести старые pod'ы из трафика: старый бинарь
продолжает читать legacy-ключ до завершения ротации.
...
## API V2 с уровнями цен

V2 наследует существующие view-классы и не меняет ответы V1. В каждый тариф
добавлено read-only поле `price_levels`, упорядоченное по `level`:

```json
{
  "price_levels": [
    {"level": 1, "price": "900.00"}
  ]
}
```

Если уровней нет, поле содержит пустой список. Остальные поля тарифа сохраняют
прежний формат.

| Эндпоинт | Методы |
| --- | --- |
| `/api/v2/listapi/tariffs/` | GET — внешний список тарифов |

Новые V2-ручки добавляются в отдельный `listapi/urls_v2.py` по мере появления
новых версий контрактов. Detail, `query_tariff` и дерево продуктов пока доступны
только в V1 и не содержат `price_levels`.
