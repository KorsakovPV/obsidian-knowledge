...
## Customers и bulk-загрузка контрактов

`OrderTransformManager.get_contract_objs()` получает контракты из Customers через
`GET /api/v1/contracts` с CSV-параметром `id__in` и `relationship=true`. Результат
преобразуется в словарь по UUID контракта и используется семью вызовами: списком
и отчётом заказов (основные и родительские контракты), test→prod, сменой контракта
и адаптером OP→ITSM.

Текущее поведение, важное для сопровождения:

- пустой `list[UUID]` сериализуется как `id__in=` и всё равно отправляется в
  Customers;
- любой non-200 и невалидный `200` в bulk-методе превращается в `{}`;
- сетевые ошибки HTTP-слой после двух попыток представляет как tuple со статусом
  `500`, поэтому вызывающий код не отличает их от других технических отказов;
- Redis-клиент синхронный внутри async-кода, а проверка cache hit основана на
  truthiness;
- `op_itsm_adapter` запускается каждую минуту, single-flight блокировки в текущем
  коде нет;
- при непустой пачке отсутствие ожидаемого контракта может привести к обращению
  к `None.parent_client_id` до фаз `resolve`/`perform`.

Это описание фиксирует текущее состояние, а не реализованное исправление.
Диагностика, контрактные ограничения и этапы изменения — в
